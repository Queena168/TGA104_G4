...
@WebServlet(urlPatterns = { "/back-end/designer_portfolio/PortfolioServlet",
		                    "/front-end/designer_protfolio/PortfolioListOne",
		                    "/front-end/designer_protfolio/PortfolioUpdate",
		                    "/front-end/designer_protfolio/UpdatedPortfolio",
		                    "/front-end/designer_protfolio/PortfolioByNo"})
@MultipartConfig()
...
//		res.setContentType("image/jpg, image/png, image/jpeg, image/gif");
		...
		if ("inputPortfolioName_For_Display".equals(action)) {
			...
			RequestDispatcher successView = req
					.getRequestDispatcher("/front-end/designer_protfolio/portfolioListAll.jsp");
			successView.forward(req, res);
		}
		...
		if ("getOne_For_Update".equals(action)) { // 來自portfolioListOne修改請求
			...
			RequestDispatcher successView = req.getRequestDispatcher("/front-end/designer_protfolio/portfolioUpdate.jsp");
			successView.forward(req, res);
		...
		if ("update".equals(action)) { // 來自portfolioUpdate.jsp的修改請求
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/front-end/designer_protfolio/portfolioUpdate.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
			...
			/*************************** 2.開始修改資料 *****************************************/
			PortfolioService portfolioSvc = new PortfolioService();
			updatePortfolioVO = portfolioSvc.updatePortfolio(portfolioNo, portfolioName, designerNo, portfolioPic1, portfolioPic2, 
			...
			RequestDispatcher successView = req.getRequestDispatcher("/front-end/designer_protfolio/portfolioListOne.jsp");
			successView.forward(req, res);
		...
		if ("portfolio_GetByNo".equals(action)) { // 來自/front-end/designer_protfolio/index.jsp點擊設計師資料的請求
			...
			/*************************** 3.查詢完成,準備轉交(Send the Success view) *************/
			req.setAttribute("portfolioListByNo", portfolioListByNo); // 資料庫取出的PortfolioVO物件,存入req
			RequestDispatcher successView = req
